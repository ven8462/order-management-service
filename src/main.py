...
from src.order_service import call_inventory_service, call_payment_service, CircuitBreakerError
...
def create_order(order: OrderData):
    try:
        ...
        if "Payment service is down" in str(e):
            raise HTTPException(status_code=500, detail="Payment service is down")
