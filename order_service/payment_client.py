...
PAYMENT_SERVICE_BASE_URL = os.getenv(
    "PAYMENT_SERVICE_URL",
    "https://nonoffensive-suasively-lorri.ngrok-free.dev",
)
...
@payment_breaker
def create_payment(  # noqa: PLR0913
    order_id: str,
    amount: float,
    items: list[dict],
    ...
    *,  # 👈 forces everything below to be keyword-only
    test_mode: bool = True,
    retry: bool = False,
) -> dict:
    payload = {
    ...
    try:
        ...
        except (ValueError, KeyError):  # 👈 specific exceptions (instead of bare Exception)
        ...
        if e.response.status_code in (429, 500, 502):
